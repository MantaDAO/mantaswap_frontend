...
const Swap: React.FC<SwapProps> = ({ route, details }) => {
  ...
  const activeFromToken = useTokenStore((state) => state.activeFromToken);
  const activeToToken = useTokenStore((state) => state.activeToToken);
  return (
    <div className="w-full max-w-[420px] flex flex-col gap-3">
          ...
          <div className="flex gap-2 items-center">
            <div className="flex items-center gap-1">
              ...
              <div className="text-sm text-body font-semibold">
                {activeFromToken?.balance} {activeFromToken?.ticker}
              </div>
            </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2 items-center">
            <div className="flex items-center gap-1">
              ...
              <div className="text-sm text-body font-semibold">
                {activeToToken?.balance} {activeToToken?.ticker}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-full">
        <JSONPretty data={activeFromToken} />
        <JSONPretty data={activeToToken} />
      </div>
    </div>
  );
};
